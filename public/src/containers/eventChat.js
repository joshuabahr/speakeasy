...
class EventChat extends Component {
      ...
      if (this.state.files.length !== 0) {
        ...
        this.state.files.map((file, index) => {
          (images[index] = Math.floor(Math.random() * 100000) + file.name),
            (imageLink[index] = `https://s3-us-west-1.amazonaws.com/mutt-match/${images[index]}`);
        });
        console.log('images and links ', images, imageLink);
        axios
          .post('/api/event/image/upload/geturl', images)
          .then(response => {
            console.log('image upload response ', response);
            response.data.map((eachFile, index) => {
              this.registerImageUrl(eachFile);
              axios.put(eachFile.url, this.state.files[index]).then(() => {
                socket.emit(
                  'newmessage',
                  {
  ...
  registerImageUrl(eachFile) {
    ...
    axios
      .post('/api/event/image/upload', imageData)
      .then(response => {})
      .catch(error => {
  ...
  handleCloseClick(event) {
    ...
    axios.put('/api/crosspath', { eventId: this.props.event.id }).then(() => {
      axios.put('/api/event/close', { event_id: this.props.event.id }).then(() => {
        this.setState({
          closed: true
        });
      });
  ...
  renderCloseEventButton() {
    ...
    if (this.props.user_id === this.props.event.userId) {
      closeEvent = (
        <button className="btnghost2" onClick={this.handleCloseClick}>
          <i className="fa" />
          Close
        </button>
      );
  ...
  renderSendButton() {
    let send = (
      <button className="btnghost2" onClick={this.handleSendClick}>
        Send
      </button>
    );
    return send;
  ...
  renderPasswordButton() {
    let send = (
      <button className="btnghost2" type="submit" value="Submit">
        Submit
      </button>
    );
    return send;
  ...
  renderCancelButton() {
    return (
      <Link to="/home">
        <button type="button" className="btnghost2">
          Cancel
        </button>
      </Link>
    );
        ...
        <div>
          ...
          <section>
            <div className="container content-section">
              <div className="row">
                <div className="container text-center row col-md-8 col-md-offset-2">{this.renderMessage()}</div>
              </div>
            </div>
          </section>

          <section id="contact">
            <ChatLog roomMessages={this.props.messages} dmClick={this.handleDMClick} />
          </section>
